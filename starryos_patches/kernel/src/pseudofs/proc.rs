...
fn task_status(task: &AxTaskRef) -> String {
    let mem = task.as_thread().proc_data.memory_stats();
    let vm_size_kb = mem.virtual_pages * PAGE_SIZE_4K / 1024;
    let vm_rss_kb = mem.resident_pages * PAGE_SIZE_4K / 1024;
    let vm_hwm_kb = mem.high_watermark_pages * PAGE_SIZE_4K / 1024;
    format!(
        "Tgid:\t{}\n\
        Pid:\t{}\n\
        VmSize:\t{} kB\n\
...
fn task_statm(task: &AxTaskRef) -> String {
    let mem = task.as_thread().proc_data.memory_stats();
    format!("{} {} 0 0 0 0 0\n", mem.virtual_pages, mem.resident_pages)
}
